...
# SYSTEM STATUS: OMEGA V119 - SUPREME SECURITY & PREMIUM MESSAGING
st.set_page_config(page_title="EMPIRE_PRO_V119", layout="wide", page_icon="🛡️")
...
LANGS = {
    "FR": {
        ...
        "save": "🚀 EXECUTE ENROLLMENT", "export": "📥 DOWNLOAD DATA", "logout": "Déconnexion",
...
# ⚡ THE SUPREME CSS (360° BORDERS & VIBRANCY)
st.markdown("""
    <style>
    ...
    div[data-baseweb="input"], div[data-baseweb="select"], div[data-baseweb="base-input"], .stDateInput div {
        ...
        background-color: #ffffff !important; padding: 4px !important;
    }
...
# --- 4. LOGIN SYSTEM (PRO ERROR PROTOCOLS) ---
            ...
            try:
                ...
                if not match.empty:
                    ...
                    if str(user_row['Status']).strip() == 'Active':
                        ...
                        st.error("🚫 **ACCÈS SUSPENDU :** Votre compte est temporairement bloqué. Veuillez contacter Fatima Elbouhali pour régulariser votre abonnement.")
                else:
                    st.error("❌ **LOGIN INCORRECT :** Identifiant business ou Access Key invalide. Veuillez réessayer.")
            except Exception as e: st.error(f"System Sync Error: {e}")
    st.stop()

# --- 5. DATA LOADING & AUTO-GENESIS ---
...
try:
    ...
    # Auto-repair empty or malformed sheets
    c_sheet_obj.clear()
    c_sheet_obj.update('A1:I1', [required_cols])
    st.rerun()
...
# PAGE RAPPELS (WOW EFFECT)
    ...
    if not urgent.empty:
        for _, r in urgent.iterrows():
            ...
            # 💡 THE WOW EFFECT MESSAGE
            pro_msg = (f"Bonjour *{r['Nom']}*,\n\n"
                       f"Votre abonnement *{r['Service']}* arrive à expiration dans *{r['Days']} jours* ⏳\n"
                       f"Date de fin : *{r['Date_Display']}* 📅\n\n"
                       f"Pour éviter toute interruption, nous vous recommandons de renouveler dès maintenant.\n\n"
                       f"Merci pour votre confiance,\n"
                       f"*{st.session_state['biz_name'].upper()}*")
...
# PAGE REÇUS
    ...
    if not df.empty:
        ...
        st.markdown(f'<div class="receipt-card"><pre style="color:white; font-size:18px; font-weight:bold; white-space: pre-wrap;">✅ REÇU - {st.session_state["biz_name"].upper()}\n👤 Client: {c["Nom"]}\n💰 Prix: {c["Prix"]} DH\n🛠️ Service: {c["Service"]}\n⌛ Expire: {c["Date_Display"]}</pre></div>', unsafe_allow_html=True)
        wa_text = f"✅ *REÇU - {st.session_state['biz_name'].upper()}*\n👤 Client: *{c['Nom']}*\n💰 Prix: *{c['Prix']} DH*\n🛠️ Service: *{c['Service']}*\n⌛ Expire: *{c['Date_Display']}*"
        st.link_button("📲 SEND VIA WHATSAPP", f"https://wa.me/{clean_num(c['Phone'])}?text={urllib.parse.quote(wa_text)}")
