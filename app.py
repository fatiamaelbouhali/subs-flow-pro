...
# SYSTEM STATUS: OMEGA V112 - ANALYTICS FIRST | LARGE SUMMARY | EXCEL RESTORED
st.set_page_config(page_title="EMPIRE_PRO_V112", layout="wide", page_icon="🛡️")
...
# ⚡ THE SUPREME CSS (BLUE BOLD METRICS & LARGE TABLES)
st.markdown("""
    <style>
    ...
    div[data-testid="stMetricLabel"] p { color: #1e3a8a !important; font-weight: 900 !important; text-transform: uppercase !important; }
...
if "auth" not in st.session_state:
    ...
    with col_log:
        sel_l_login = st.selectbox("Language / اللغة", ["FR", "AR"], key="log_lang")
        u_in = st.text_input(LANGS[sel_l_login]["ident"])
        p_in = st.text_input(LANGS[sel_l_login]["pass"], type="password")
        if st.button(LANGS[sel_l_login]["btn_log"], use_container_width=True):
