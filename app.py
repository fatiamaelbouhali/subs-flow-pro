...
# SYSTEM STATUS: OMEGA V116 - ROBUST LOGIN & AUTO-MAPPING
st.set_page_config(page_title="EMPIRE_PRO_V116", layout="wide", page_icon="🛡️")
...
# --- 4. LOGIN SYSTEM (WITH ERROR MESSAGES) ---
    ...
    with col_log:
        ...
        if st.button(L_log["btn_log"], use_container_width=True):
            try:
                ...
                # 💡 FORCE STRIP & STRING FOR MATCHING
                ...
                if not match.empty:
                    ...
                    if str(user_row['Status']).strip() == 'Active':
                        st.session_state.update({
                            "auth": True, "user": u_in, "lang": sel_l_login, 
                            "biz_name": str(user_row['Business_Name']), 
                            "sheet_name": str(user_row['Sheet_Name'])
                        })
                        st.rerun()
                    else:
                        st.error("🚫 Access Suspended. Contact Fatima.")
                else:
                    st.error("❌ Identifiants incorrects (Username or Key).")
