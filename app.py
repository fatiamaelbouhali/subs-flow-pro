...
# SYSTEM STATUS: OMEGA V118 - AUTO-GENESIS ARCHITECTURE (NO MORE EMPTY SHEET ERRORS)
st.set_page_config(page_title="EMPIRE_PRO_V118", layout="wide", page_icon="🛡️")
...
# --- 5. DATA LOADING & AUTO-GENESIS (V118) ---
...
try:
    ...
    # 💡 AUTO-INJECT HEADERS IF SHEET IS EMPTY
    if not data_raw:
        c_sheet_obj.update('A1:I1', [required_cols])
        df = pd.DataFrame(columns=required_cols)
    else:
        df = pd.DataFrame(data_raw)
        # Force repair headers
    ...
    # 💡 ULTIMATE RECOVERY: If Google returns [200] but fails parsing, we reset the headers
    try:
        c_sheet_obj.clear()
        c_sheet_obj.update('A1:I1', [required_cols])
        st.info("🔄 Base auto-repaired. Please refresh.")
        st.rerun()
    except:
        st.error(f"Fatal Sync Error: {e}"); st.stop()
